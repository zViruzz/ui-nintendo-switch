	...
	return (
		<AnimatePresence>
			{!isHiddenMenu && (
				<motion.section
					id={title}
					className='absolute top-0 h-screen w-screen left-0 flex'
					initial={{ opacity: 0 }}
					animate={{ opacity: 1 }}
					exit={{ opacity: 0 }}
					...
					<div className='bg-[#484848] grow grid grid-rows-[8fr_1fr]'>
						<div className='mt-20 px-[8.7rem] TransitionOpacitySlide' key={id}>
							<div className='flex gap-12'>
								<img className='w-[24rem] h-[24rem]' src={url} alt={title} />
								<div className=' w-full flex flex-col'>
									<h2 className='text-5xl grow mt-[6.5rem] px-10'>{title}</h2>
									<button
										id={id}
